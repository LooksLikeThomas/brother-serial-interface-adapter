...
| Code+7 | `0x88,0x37,0x89` | `0x5E` | `^` |
| Code+8 | `0x88,0x38,0x89` | `0x1B, 0x59` | ESC+Y (wheel probe) |
| Code+9 | `0x88,0x39,0x89` | `0x3C` | `<` |
| Code+0 | `0x88,0x30,0x89` | `0x3E` | `>` |

Code+1 through Code+5 produce the remaining ASCII control codes (0x1B–0x1F). Code+7 through Code+0 produce printable characters that are otherwise unavailable on the keyboard. Code+6 is a gap (no output). Code+8 produces the ESC+Y escape sequence (wheel position probe, see §15).
...
#### 16.9.5 Code Key Is KB-Independent

The Code+key → control code mapping is identical across all three keyboard positions. It uses a fixed algorithm (`letter & 0x1F` for letters, fixed assignments for number row) rather than the keyboard-dependent translation table.
...
| Code+8 | `0x1B, 0x59` (ESC+Y) | Wheel probe — print glyph at space position |
